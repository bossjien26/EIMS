...
{
    [TestFixture]
    public class ClientContactRepositoryTest : BaseRepositoryTest
    {
        private readonly IClientContactRepository _repository;

        public ClientContactRepositoryTest() => _repository = new ClientContactRepository(_context);

        [Test]
        async public Task ShouldGet()
        {
            var testData = ClientContactSeeder.SeedOne();
        ...
        {
            await _repository.InsertMany(ClientContactSeeder.SeedMany(5, 5));
        ...
        {
            var data = ClientContactSeeder.SeedOne();
